...
async function canViewDiagram(diagramId: string, userId: string): Promise<boolean> {
...
async function canEditDiagram(diagramId: string, userId: string): Promise<boolean> {
  const own = await getDb().execute({
    sql: 'SELECT 1 FROM diagrams WHERE id = ? AND owner_id = ? LIMIT 1',
    args: [diagramId, userId],
  })
...
diagrams.get('/:id/access', async (c) => {
  ...
  if (!(await canViewDiagram(id, s.sub))) return c.json({ error: 'No encontrado' }, 404)
  const owner = await getDb().execute({
    sql: `SELECT u.id, u.email, u.name, 'owner' AS role FROM diagrams d JOIN users u ON u.id = d.owner_id WHERE d.id = ?`,
    args: [id],
  })
  const collab = await getDb().execute({
...
diagrams.post('/:id/access', async (c) => {
  ...
  const ownerRow = await getDb().execute({
...
diagrams.post('/:id/demo', async (c) => {
  ...
  if (!s) return c.json({ error: 'No autorizado' }, 401)
  const diagramId = c.req.param('id')
  if (!(await canEditDiagram(diagramId, s.sub))) return c.json({ error: 'No autorizado' }, 403)
...
diagrams.put('/:id', async (c) => {
  ...
  if (!s) return c.json({ error: 'No autorizado' }, 401)
  const id = c.req.param('id')
  if (!(await canEditDiagram(id, s.sub))) return c.json({ error: 'No autorizado' }, 403)
